...
- [ ] G7 (§4): `POST /repo/upload` — accept a browser-picked local folder, reconstruct + `git init` server-side, wire the landing page's Browse tile to it. Frontend side (native `webkitdirectory` picker, honest "not wired yet" notice) already shipped in Phase 2 ([components/landing/RepoPicker.tsx](frontend/components/landing/RepoPicker.tsx)) — still an open gap, not a chat feature
