...
| G8 | The model selector (Phase 2, `GET /llm/providers`) only affected the one LLM call the landing page makes (`POST /repo/{id}/discover`) — no effect on execution-time calls | **✅ Landed & live**, and since revised from provider-level to model-level selection. Nullable `provider` column on `seams` (`schema.sql`, stores a specific **model string**, not a provider name — see below); `SeamIn.model`/`DiscoverIn.model` persisted/threaded through `POST /repo/{id}/seam` → `POST /campaign` → `verification/gate.py` → `execution/codex.py`'s `migrate_file(..., model=...)` → `llm.complete(model=...)`, so the exact model picked drives **every** execution/retry call for that campaign, not just the one planning call. Verified live end-to-end (real, non-mock Groq): created a seam with `model="llama-3.1-8b-instant"`, ran a real campaign, confirmed the unit's `codex_rationale` event was genuine LLM prose from that specific model. |
| G8b | Follow-up: the selector only ever offered one hardcoded model per provider (`CODEX_MODEL`/`GROQ_MODEL` env defaults) — no real choice, and no sense of relative cost/quota impact (directly relevant after this session hit Groq's daily token quota from testing) | **✅ Landed.** `llm.py` gained a curated `_MODEL_CATALOG` (4 models each for codex/groq, each tagged `usage: "low"\|"mid"\|"high"` — a static, hand-maintained tier, not a live quota reading) and `list_models()`/`_provider_for_model()` to resolve a specific model string back to its provider's credentials; `active_provider()`/`complete()`/`complete_json()` renamed their override param `provider_name` → `model` throughout (`execution/codex.py`, `planning/seam_discovery.py` included) to match. `GET /llm/providers` now returns `{providers: [{name, models: [{model, usage}]}], active}` (was `{name, model}` per provider) — `models.LlmModelOut`/`LlmProviderOut` updated to match; `active` is now the active **model** string. `ModelMenu.tsx` rewritten: groups models by provider, shows a usage badge per model (reusing the existing queued/retry/fail token colors — no new palette), selecting one sets the exact model. Verified live: `GET /llm/providers` returns 4 real groq models with usage tiers (only `OPENAI_API_KEY` being unset is why codex doesn't show); picked the "low" usage model explicitly, ran a real campaign end to end on it, confirmed via the `codex_rationale` event. `tsc --noEmit` + `next lint` clean. |
